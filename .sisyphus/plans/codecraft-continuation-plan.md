...
Berdasarkan analisis dari 4 agents (Metis, Explore, Librarian x2), berikut adalah roadmap untuk melanjutkan pengembangan Codecraft dengan referensi dari **OpenCode** (https://github.com/anomalyco/opencode).
...
|-----|----------|-------------------|
| **ZERO TESTS** | 🔴 Critical | Aider has extensive mocks |
| **API Key Bug** | 🔴 Critical | Agent ignores stored config |
| **No Session Persistence** | 🟡 High | OpenCode has full session lifecycle |
| **No Context Management** | 🟡 High | OpenCode has compaction agent |
| **No Token Tracking** | 🟡 Medium | Aider has `/tokens` command |
| **Desktop Not Integrated** | 🟡 Medium | OpenCode uses client/server |
...
*Reference: OpenCode compaction agent, Aider `/tokens`*
...
*Reference: OpenCode client/server pattern*
...
*Reference: OpenCode plugin system, MCP*
...
| Risk | Mitigation |
|------|------------|
| Zero tests | TDD approach - tests first |
| API key bug | Fix immediately (T0) |
...
*Reference: OpenCode (anomalyco/opencode)*
*Agents used: Metis, Explore, Librarian x2*
